...
Для предотвращения race condition используются блокировки строк остатков.
...
Демонстрационный сервис `restaurant-example` запускается вместе с основным приложением через Docker Compose и используется для проверки взаимодействия между платформой и заведением.
...
Товар также содержит `establishment_id`. Это позволяет напрямую получать товары конкретного заведения и использовать индекс:

```text
products_establishment_idx
```
...
```text
product 1 ─── N modifiers
```

Каждый модификатор содержит варианты:

```text
modifier 1 ─── N modifier_options
```
...
`price_delta` определяет изменение стоимости товара при выборе конкретного варианта.
...
```text
stocks
 ├── product_id  → остаток товара
 │
 └── option_id   → остаток варианта модификатора
```
...
Для каждого заказа хранится история его состояний:
...
История позволяет восстановить последовательность изменения статусов и время каждого перехода.
...
Старый заказ продолжает содержать:
...
Для рублей используются копейки:

```text
59900 = 599 рублей
```

Это позволяет избежать ошибок округления, характерных для операций с `float`.
...
API разделён на две логические части:
...
Схема базы данных создаётся с помощью миграций Goose.

Миграции находятся в репозитории и содержат как создание таблиц, так и их удаление в `Down`-части.
...
После запуска основной API доступен на:
...
* платежи;
* полноценная курьерская система;
* геолокация;
* временные интервалы доставки;
* мультирегиональность;
* rate limiting;
* гарантированная доставка webhook;
* распределённая трассировка;
* очередь сообщений;
* production-ready мониторинг.

Эти ограничения позволяют сосредоточиться на основной бизнес-логике MVP.
